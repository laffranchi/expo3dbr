...
import { Loader2, Eye, CheckCircle2, XCircle, Store } from "lucide-react";

interface ExhibitorSubmission {
  id: string;
...
const ExhibitorsManagement = () => {
  ...
  const [submissions, setSubmissions] = useState<ExhibitorSubmission[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedSubmission, setSelectedSubmission] = useState<ExhibitorSubmission | null>(null);
  ...
  const fetchSubmissions = async () => {
    try {
      let query = supabase
        ...
        .select("*")
        .in("categoria", ["expositor", "patrocinador"])
  ...
  const handleViewDetails = (submission: ExhibitorSubmission) => {
    setSelectedSubmission(submission);
    setDetailsOpen(true);
  };

  const handleApprove = (submission: ExhibitorSubmission) => {
    setSelectedSubmission(submission);
    setApprovalType("approve");
    setApprovalOpen(true);
  };

  const handleReject = (submission: ExhibitorSubmission) => {
    setSelectedSubmission(submission);
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">Expositores & Patrocinadores</h1>
            <p className="text-muted-foreground">
              Gerencie as propostas de expositores e patrocinadores
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Submissões de Expositores</CardTitle>
            <CardDescription>
                  ...
                  <TableBody>
                    {submissions.map((submission) => (
                      <TableRow key={submission.id}>
                        ...
                        <TableCell>{getCategoryLabel(submission.categoria)}</TableCell>
                        <TableCell>
        ...
        <SubmissionDetailsDialog
          ...
          fields={[{ key: "categoria", label: "Categoria" }, ...exhibitorFields]}
          onApprove={() => {
            setApprovalType("approve");
            setApprovalOpen(true);
          }}
